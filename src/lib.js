...
async function getItems(base64){
    ...
    for(let item of items){
        ...
        const customTextures = await customResources.getTextures(item);

        if(customTextures.length > 0){
            const customTexture = customTextures[0];

            if(customTexture){
                item.animated = customTexture.animated;
                item.texture_path = '/' + customTexture.path;
                item.texture_pack = customTexture.packId;
                item.custom_textures = customTextures;
            }
        }
